...
module "synapse" {
  ...
  storage_account_key        = module.data_storage.storage_account_key
  ...
  azure_monitor_id           = module.azure_monitor.id
  tags                       = var.tags
  storage_sas_token          = module.data_storage.storage_sas
  primary_blob_endpoint      = module.data_storage.primary_blob_endpoint
...
resource "azurerm_resource_group_template_deployment" "customerattribution" {
  count               = var.deploy_customer_attribution ? 1 : 0
  name                = "pid-${var.partner_id}"
  depends_on          = [azurerm_resource_group.eiad]
  resource_group_name = azurerm_resource_group.eiad.name
